...
export async function POST(request: Request) {
  try {
    ...
    const rl = checkRateLimit(`generate:${userId}`, { max: 5, windowSec: 60 });
    if (!rl.allowed) {
      logger.warn('generate', 'Rate limited', { userId });
    ...
    const limitCheck = await withRetry(() => canCreateApp(userId));
    if (!limitCheck.allowed) {
      logger.info('generate', 'Plan limit reached', { userId, current: limitCheck.current, limit: limitCheck.limit });
