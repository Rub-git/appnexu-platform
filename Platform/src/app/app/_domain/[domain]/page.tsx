...
export async function generateMetadata({ params }: { params: Promise<{ domain: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    icons: {
      icon: [
        { url: getAppIconUrl(app.id, 192, assetVersion), sizes: '192x192', type: 'image/png' },
        { url: getAppIconUrl(app.id, 512, assetVersion), sizes: '512x512', type: 'image/png' },
      ],
      apple: [{ url: getAppIconUrl(app.id, 180, assetVersion), sizes: '180x180', type: 'image/png' }],
    },
    openGraph: {
      title: app.appName,
